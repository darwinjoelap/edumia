...
## D-34 — Fase 8: "Todos los ingresos" y "Todos los gastos" (historial filtrable por estado)
- Estado: Cerrada (2026-09-25)
- Origen: Darwin, a punto de compartir el sistema con la administradora, pidió poder ver todos los ingresos y filtrarlos por estado (y lo mismo para gastos) — "esto facilita por si el ingreso lo verifico y no recuerda el numero de cedula". Hasta ahora no había ninguna pantalla que mostrara aportes/gastos en **cualquier** estado: la bandeja de verificación (`aporte_bandeja`) solo muestra `registrado`/`observado`, "Buscar aporte" (`aporte_buscar`) exige coincidencia exacta de referencia/cédula/teléfono, y la bandeja de aprobación de gastos (`gasto_bandeja`) solo muestra `registrado`. Nada cubría "ya lo verifiqué, ¿dónde quedó?".
- **Piezas nuevas:** `AporteListView` (`ingresos/views.py`, URL `ingresos/lista/`, plantilla `aporte_lista.html`) y `GastoListView` (`gastos/views.py`, URL `gastos/lista/`, plantilla `gasto_lista.html`) — mismo patrón que `BitacoraListView` (D-28): `ListView` con `get_queryset()` filtrando por `?estado=` (exacto) y `?q=` (búsqueda libre), paginado de a 40, y `get_context_data()` exponiendo las opciones de estado para el `<select>` del formulario. La búsqueda de ingresos reutiliza la misma normalización de cédula/teléfono/referencia que "Buscar aporte" (D-19), y además busca por nombre/apellido del estudiante y por "entregado por" — así encuentra un aporte aunque no se recuerde ninguno de los tres datos exactos. La de gastos busca por proveedor, número de documento o fondo.
- **Quién puede ver el historial completo (`ROLES_HISTORIAL`, nuevo en ambas apps):** administrador y responsable de fondo (los mismos que ya registran/verifican/aprueban) más **director y auditor** — que hasta ahora solo tenían acceso a Reportes y balance (solo consulta) y no podían ver el detalle de un aporte o gasto individual. Se amplió también `gasto_detalle` (antes solo administrador/responsable de fondo) y `recibo_detalle` (`recibos/views.py`, antes los mismos roles que veían un aporte) al mismo grupo, para que "Ver" y "Ver recibo" desde la lista nueva no den 403 — los botones de acción (aprobar, editar, anular) siguen ocultos para director/auditor porque dependen de `puede_aprobar`/`puede_editar`, que no cambiaron.
- El menú lateral (`templates/base.html`) gana un enlace "Todos los ingresos"/"Todos los gastos" en cada sección, visible para los cuatro roles; los enlaces de registrar/bandeja/buscar siguen viéndose solo para administrador y responsable de fondo, como antes.
- No se tocó ningún modelo — no hizo falta ninguna migración.
- Validado en sandbox: `manage.py check`, `makemigrations --check --dry-run` sin cambios, la suite existente (15 tests) sigue en verde, y un script de 19 verificaciones nuevas: acceso por rol a ambas listas (administrador/responsable de fondo/director/auditor → 200, docente → 403), sin filtro trae todos los estados, filtro por estado exacto, búsqueda por cédula (ingresos) y por proveedor (gastos), y que director ahora puede ver el detalle de un gasto pero sigue sin ver el botón de aprobar.

## D-35 — Fase 8: categoría "libre" en un renglón de gasto sin producto en el catálogo
- Estado: Cerrada (2026-09-25)
- Origen: Darwin notó que, al registrar un gasto, si el producto no está en el catálogo (se usa el campo "descripción" en vez de elegir un `Producto`), ese renglón no tenía ninguna categoría asociada — porque hasta ahora la categoría siempre salía de `producto.categoria`, y sin producto no había de dónde sacarla. Eso hacía que el reporte "Gastos por categoría y producto" agrupara todos esos renglones en un cajón genérico "Sin categoría / sin producto", perdiendo la clasificación aunque Darwin supiera perfectamente a qué categoría pertenecía el gasto.
- **Diseño — opcional, decidido con Darwin por `AskUserQuestion`:** se agregó un campo `categoria` (FK a `CategoriaGasto`, nula) directo en `DetalleGasto`, que solo tiene sentido cuando no hay `producto` — si el renglón sí tiene producto, `DetalleGasto.save()` limpia ese campo solo (la categoría real es la del producto, nunca las dos guardadas a la vez). Es opcional a propósito: Darwin prefirió no obligar a elegir categoría en cada renglón libre; un renglón sin categoría elegida simplemente sigue cayendo en "Sin categoría / sin producto", como antes.
- **Reportes (`reportes/services.py::gastos_por_categoria_producto`):** se cambió el agrupamiento de `producto__categoria` a una categoría "efectiva" calculada con `Coalesce(producto__categoria, categoria)` — así un renglón de descripción libre con categoría elegida se agrupa junto a los demás gastos de esa categoría (sumando su monto al total), y el filtro por categoría del reporte también lo encuentra. Solo cuando no hay ni producto ni categoría elegida cae en el cajón genérico.
- **Piezas tocadas:** `gastos/models.py` (campo nuevo + limpieza en `save()`), `gastos/forms.py` (`DetalleGastoForm` gana el campo, opcional, con las categorías activas), `gastos/templates/gastos/_renglon_fila.html` y `gasto_form.html` (columna nueva + texto de ayuda actualizado), `gastos/templates/gastos/gasto_detalle.html` (muestra la categoría del renglón, sea del producto o la elegida a mano), `gastos/views.py` (`select_related` ampliado en `gasto_detalle`), `reportes/services.py`, `reportes/views.py` (las claves de exportación a Excel/PDF cambian de `producto__categoria__nombre` a `categoria_efectiva_nombre`) y `reportes/templates/reportes/gastos_categorias.html` (el `regroup` usa la nueva clave).
- Migración: `gastos/migrations/0005_detallegasto_categoria_and_more.py` (agrega el campo a `DetalleGasto` y a su tabla histórica de `simple-history`).
- Validado en sandbox: `manage.py check`, `makemigrations --check --dry-run` limpio tras generar la migración, `migrate` aplicado, la suite existente (15 tests) sigue en verde, y un script de 9 verificaciones nuevas: un renglón con producto limpia sola cualquier categoría libre que traiga, un renglón de descripción libre guarda su categoría elegida, uno sin categoría elegida sigue siendo válido (es opcional), uno sin producto y sin descripción sigue bloqueado aunque tenga categoría (la regla vieja no cambió), y el reporte agrupa correctamente: el renglón libre con categoría suma al total de esa categoría (no aparece en "Sin categoría"), y el filtro por categoría del reporte también lo encuentra.

## D-36 — Fase 8: transferencias entre fondos
- Estado: Cerrada (2026-09-25)
- Origen: Darwin pidió poder "movilizar fondos entre fondo, por ejemplo, general a caja chica" — hasta ahora no había ninguna forma de mover dinero de un fondo a otro dentro de Edumia; solo se podía cargar el saldo inicial de cada uno por separado (D-32).
- **Alcance decidido con Darwin (por `AskUserQuestion`):** una moneda por movimiento (Bs. o USD, nunca las dos juntas ni convertidas entre sí — mismo criterio de todo el sistema, D-02); solo el rol administrador puede registrar transferencias (mismo rol que ya administra fondos); se puede anular una transferencia ya hecha (con motivo obligatorio, revierte el efecto en el saldo de los dos fondos); y se permite dejar el fondo de origen en saldo negativo, con una confirmación explícita — igual que aprobar un gasto que deja el fondo en negativo (D-05/G-3).
- **Diseño — modelo propio, no un `Aporte`/`Gasto` fabricado:** una transferencia no es un ingreso ni un gasto real (no entra ni sale dinero de la institución, solo cambia de fondo), así que crear un `Aporte`/`Gasto` falso para moverla habría ensuciado los reportes y las listas de "Todos los ingresos"/"Todos los gastos" (D-34) con movimientos que no son transacciones reales. Se agregó `gastos/models.py::TransferenciaFondo`: `fondo_origen`, `fondo_destino`, `monto`, `moneda`, `fecha`, `motivo`, con una máquina de estados de solo dos pasos (`registrada` → `anulada`, sin aprobación intermedia porque ya es admin quien la registra directamente).
- **Validación en `TransferenciaFondo.clean()`** (no en la vista, mismo patrón que `Gasto`/`DetalleGasto`): fondo de origen y destino deben ser distintos (además reforzado con un `CheckConstraint` a nivel de base de datos), y si la transferencia deja el fondo de origen en negativo en esa moneda, exige el campo `saldo_negativo_confirmado` marcado — si no, el formulario muestra el saldo proyectado como parte del mensaje de error.
- **`gastos/services.py::saldo_fondo()`** se amplió: además de `saldo_inicial + aportes verificados − gastos aprobados`, ahora suma las transferencias entrantes y resta las salientes (solo las que están `registrada`, nunca las `anulada`) — un solo cálculo, sigue siendo la única fuente de verdad del saldo de un fondo (dashboard, reportes y esta pantalla nueva lo usan igual).
- **Piezas nuevas:** `TransferenciaFondoForm` (`gastos/forms.py`, deja que `clean()` del modelo hable), `TransferenciaFondoListView`, `transferencia_registrar`, `transferencia_anular` (`gastos/views.py`, `@requiere_rol("administrador")`), tres plantillas (`transferencia_form.html`, `transferencia_lista.html`, `transferencia_motivo.html` — esta última copia del patrón ya usado por `gasto_motivo.html`). Enlace nuevo "Transferir entre fondos" en el menú lateral (solo administrador) y un botón "Transferencias" en la pantalla de Fondos.
- **Auditoría:** nuevo evento `transferir_fondo`; la anulación reutiliza el evento genérico `anular` ya existente (como con Aporte/Gasto), diferenciado por `modelo="TransferenciaFondo"` — migración `core/migrations/0008_alter_registroauditoria_accion.py` (solo cambia los `choices`, igual que las anteriores).
- Migración de modelo: `gastos/migrations/0006_historicaltransferenciafondo_transferenciafondo.py` (crea `TransferenciaFondo` y su tabla histórica de `simple-history`).
- Validado en sandbox: `manage.py check`, `makemigrations --check --dry-run` limpio tras las dos migraciones, `migrate` aplicado, la suite existente (15 tests) sigue en verde, y un script de 15 verificaciones nuevas: permisos (docente sin acceso a registrar ni a listar, 403), una transferencia válida mueve el saldo correcto en la moneda elegida sin tocar la otra, origen y destino iguales se rechaza, saldo insuficiente sin confirmar se rechaza y confirmado se acepta (dejando el fondo en negativo), anular revierte el efecto exactamente en el fondo de origen (verificado con un segundo movimiento de por medio, para confirmar que la resta es la correcta y no un simple "volver al valor de antes"), y no se puede anular una transferencia ya anulada.
