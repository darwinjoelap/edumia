...
- [ ] **Diseño acordado, no implementado todavía** — Promoción masiva con asignación manual de sección destino: el administrador confirmó que el personal arma las secciones a mano cada año (no hay regla automática de "mismo nombre" ni reparto equilibrado). Reemplaza el emparejamiento automático por nombre de `academico/promocion.py` (`calcular_plan`/`ejecutar_promocion`) por:
  1. Paso 1 (sin cambios): elegir período destino.
  2. Paso 2 (nuevo): por cada sección de origen con alumnos activos, listar los estudiantes con un `<select>` de "sección destino" (opciones: secciones del grado siguiente en el período destino, o "Egresar" si es el último grado); preseleccionar por nombre igual cuando exista (mismo comportamiento de hoy como sugerencia, no como regla); botón "aplicar a todos los de esta sección" para fijar en lote y luego poder corregir casos individuales a mano.
  3. Confirmar: crea una `Inscripcion` por estudiante en la sección elegida (o marca `egresado`); si algún estudiante quedó sin sección elegida, no se bloquea todo el lote — se avisa cuál(es) quedaron pendientes y se puede volver a correr la promoción después para completarlos (mismo criterio idempotente de hoy: a quien ya tiene inscripción en el período destino no se le vuelve a tocar).
  - Nota de implementación: con muchos estudiantes por sección esto es una tabla grande; conviene procesar el POST por `estudiante_id` → `seccion_destino_id` en vez de un formset de Django (más simple de validar en lote). Guardar la selección en sesión igual que hoy mientras se confirma.
  - Pendiente: decidir si esto se construye ahora o se dedica una ronda aparte (es un cambio no trivial al flujo existente).
